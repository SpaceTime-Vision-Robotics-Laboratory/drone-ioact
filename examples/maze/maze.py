...
class Maze:
    """Maze implementation"""
    def __init__(self, maze_size: tuple[int, int], walls_prob: float, max_tries: int):
    ...
    def move_player(self, direction: str) -> bool:
        ...
        assert not self.is_finished(), "maze already finished"
        self.n_moves += 1
        delta = {
            "up": (-1, 0),
            "down": (1, 0),
    ...
    def get_state(self) -> dict[str, PointIJ | float]:
        ...
        if (diff := (1 / FREQUENCY) - ((now := datetime.now()) - self._prev_time).total_seconds()) > 0:
            time.sleep(diff)
        distance_to_exit = np.linalg.norm(self.exit_pos - self.player_pos, ord=1).item()
        return {"position": self.player_pos, "distance_to_exit": distance_to_exit, "timestamp": now.isoformat()}
